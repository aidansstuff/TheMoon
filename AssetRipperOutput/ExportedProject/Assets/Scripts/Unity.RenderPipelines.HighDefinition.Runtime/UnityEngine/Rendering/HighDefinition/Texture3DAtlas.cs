...
namespace UnityEngine.Rendering.HighDefinition
{
	internal class Texture3DAtlas
	{
		...
		private void CopyMip(CommandBuffer cmd, Texture source, int sourceMip, Texture destination, Vector3Int destinationOffset, int destinationMip)
		{
			...
			cmd.SetComputeFloatParam(m_Texture3DAtlasCompute, HDShaderIDs._AlphaOnlyTexture, flag ? 1 : 0);
			int num = source.width >> sourceMip;
			cmd.SetComputeIntParam(m_Texture3DAtlasCompute, HDShaderIDs._SrcSize, num);
		...
		private void GenerateMip(CommandBuffer cmd, Texture source, Vector3Int sourceOffset, int sourceMip, Texture destination, Vector3Int destinationOffset, int destinationMip)
		{
			...
			cmd.SetComputeVectorParam(m_Texture3DAtlasCompute, HDShaderIDs._SrcScale, one);
			cmd.SetComputeVectorParam(m_Texture3DAtlasCompute, HDShaderIDs._SrcOffset, vector);
			cmd.SetComputeFloatParam(m_Texture3DAtlasCompute, HDShaderIDs._SrcMip, sourceMip);
			...
			cmd.SetComputeVectorParam(m_Texture3DAtlasCompute, HDShaderIDs._DstOffset, (Vector3)vector3Int);
		...
		{
			long num = (long)((float)((long)elementSize * (long)elementSize * elementSize * HDUtils.GetFormatSizeInBytes(format)) * (hasMipMaps ? 1.33f : 1f));
