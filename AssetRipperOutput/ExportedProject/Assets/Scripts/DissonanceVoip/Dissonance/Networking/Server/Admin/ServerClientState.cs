...
namespace Dissonance.Networking.Server.Admin
{
	internal class ServerClientState<TServer, TClient, TPeer> : IServerClientState where TServer : BaseServer<TServer, TClient, TPeer> where TClient : BaseClient<TServer, TClient, TPeer> where TPeer : struct, IEquatable<TPeer>
	{
		...
		public void InvokeOnEnteredRoom(string name)
		{
			...
			Action<IServerClientState, string> onStartedListeningToRoom = this.OnStartedListeningToRoom;
			if (onStartedListeningToRoom != null)
			{
				try
				{
					onStartedListeningToRoom(this, name);
				}
				catch (Exception p)
				{
		...
		{
			_rooms.Remove(name);
			Action<IServerClientState, string> onStoppedListeningToRoom = this.OnStoppedListeningToRoom;
			if (onStoppedListeningToRoom != null)
			{
				try
				{
					onStoppedListeningToRoom(this, name);
				}
				catch (Exception p)
				{
					Log.Error("Exception encountered invoking `PlayerJoined` event handler: {0}", p);
