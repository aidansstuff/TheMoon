...
namespace Dissonance.Networking.Client
{
	internal class EventQueue
	{
		private enum EventType
		{
			...
			PlayerEnteredRoom = 2,
			PlayerExitedRoom = 3,
			PlayerStartedSpeaking = 4,
			PlayerStoppedSpeaking = 5,
			VoiceData = 6,
			TextMessage = 7
		}
		...
		public bool DispatchEvents(DateTime? utcNow = null)
		{
			...
			{
				NetworkEvent @event = value[i];
				switch (@event.Type)
				{
				case EventType.PlayerJoined:
					flag |= InvokeEvent(@event.PlayerName, @event.CodecSettings, this.PlayerJoined);
					break;
				case EventType.PlayerLeft:
					flag |= InvokeEvent(@event.PlayerName, this.PlayerLeft);
					break;
				case EventType.PlayerStartedSpeaking:
					flag |= InvokeEvent(@event.PlayerName, this.PlayerStartedSpeaking);
					break;
				case EventType.PlayerStoppedSpeaking:
					flag |= InvokeEvent(@event.PlayerName, this.PlayerStoppedSpeaking);
					break;
				case EventType.VoiceData:
				{
					flag |= InvokeEvent(@event.VoicePacket, this.VoicePacketReceived);
					_pendingVoicePackets--;
					if (@event.VoicePacket.Channels != null)
					{
						@event.VoicePacket.Channels.Clear();
						_channelsListPool.Recycle(@event.VoicePacket.Channels);
					}
					byte[] array = @event.VoicePacket.EncodedAudioFrame.Array;
					if (array != null)
					{
				...
				case EventType.TextMessage:
					flag |= InvokeEvent(@event.TextMessage, this.TextMessageReceived);
					break;
				case EventType.PlayerEnteredRoom:
				{
					RoomEvent arg2 = CreateRoomEvent(@event, joined: true);
					flag |= InvokeEvent(arg2, this.PlayerEnteredRoom);
					break;
				}
				case EventType.PlayerExitedRoom:
				{
					RoomEvent arg = CreateRoomEvent(@event, joined: false);
		...
		{
			RoomEvent result = default(RoomEvent);
			result.PlayerName = @event.PlayerName;
			result.Room = @event.Room;
			result.Joined = joined;
			result.Rooms = @event.AllRooms;
			return result;
		}
