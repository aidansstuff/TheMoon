...
namespace Unity.Netcode
{
	public ref struct BitWriter
	{
		...
		public unsafe void WriteBit(bool bit)
		{
			...
			m_BitPosition++;
			m_BufferPointer[bytePosition] = (byte)(bit ? ((m_BufferPointer[bytePosition] & ~(1 << num)) | (1 << num)) : (m_BufferPointer[bytePosition] & ~(1 << num)));
		}
		...
		private unsafe void WritePartialValue<T>(T value, int bytesToWrite, int offsetBytes = 0) where T : unmanaged
		{
			...
			UnsafeUtility.MemCpy(m_BufferPointer + BytePosition, source, bytesToWrite);
