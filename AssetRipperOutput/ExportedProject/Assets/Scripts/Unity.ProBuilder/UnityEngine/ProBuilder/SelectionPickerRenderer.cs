	...
	internal static class SelectionPickerRenderer
	{
		...
		internal class SelectionPickerRendererHDRP : ISelectionPickerRenderer
		{
			public Texture2D RenderLookupTexture(Camera camera, Shader shader, string tag, int width = -1, int height = -1)
			{
				...
				HDAdditionalCameraData hDAdditionalCameraData = gameObject.AddComponent<HDAdditionalCameraData>();
				hDAdditionalCameraData.flipYMode = HDAdditionalCameraData.FlipYMode.ForceFlipY;
				hDAdditionalCameraData.customRender += CustomRenderPass;
			...
			private static void CustomRenderPass(ScriptableRenderContext ctx, HDCamera camera)
			{
				...
				ctx.Submit();
				DrawingSettings drawingSettings = default(DrawingSettings);
				drawingSettings.SetShaderPassName(0, new ShaderTagId("Always"));
