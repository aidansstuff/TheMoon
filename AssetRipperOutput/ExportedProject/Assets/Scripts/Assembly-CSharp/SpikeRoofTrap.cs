...
public class SpikeRoofTrap : NetworkBehaviour
{
	...
	public void ToggleSpikesServerRpc(bool enabled)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			if (__rpc_exec_stage != __RpcExecStage.Server && (networkManager.IsClient || networkManager.IsHost))
			{
				...
				FastBufferWriter bufferWriter = __beginSendServerRpc(1039429333u, serverRpcParams, RpcDelivery.Reliable);
				bufferWriter.WriteValueSafe(in enabled, default(FastBufferWriter.ForPrimitives));
				__endSendServerRpc(ref bufferWriter, 1039429333u, serverRpcParams, RpcDelivery.Reliable);
			}
	...
	public void ToggleSpikesClientRpc(bool enabled)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			if (__rpc_exec_stage != __RpcExecStage.Client && (networkManager.IsServer || networkManager.IsHost))
			{
				...
				FastBufferWriter bufferWriter = __beginSendClientRpc(3844133644u, clientRpcParams, RpcDelivery.Reliable);
				bufferWriter.WriteValueSafe(in enabled, default(FastBufferWriter.ForPrimitives));
				__endSendClientRpc(ref bufferWriter, 3844133644u, clientRpcParams, RpcDelivery.Reliable);
			}
			if (__rpc_exec_stage == __RpcExecStage.Client && (networkManager.IsClient || networkManager.IsHost) && trapActive != enabled)
			{
				ToggleSpikesEnabledLocalClient(enabled);
			}
		}
	...
	private bool GetNearEntrance()
	{
		...
		if (flag)
		{
			...
			{
				if (!array[j].entrancePoint == (bool)nearEntrance.exitPoint)
				{
	...
	public void SpikeTrapSlamServerRpc(int playerWhoTriggered)
	{
		...
		{
			ServerRpcParams serverRpcParams = default(ServerRpcParams);
			FastBufferWriter bufferWriter = __beginSendServerRpc(4061245259u, serverRpcParams, RpcDelivery.Reliable);
			BytePacker.WriteValueBitPacked(bufferWriter, playerWhoTriggered);
			__endSendServerRpc(ref bufferWriter, 4061245259u, serverRpcParams, RpcDelivery.Reliable);
		}
		if (__rpc_exec_stage == __RpcExecStage.Server && (networkManager.IsServer || networkManager.IsHost))
		{
	...
	public void SpikeTrapSlamClientRpc(int playerWhoTriggered)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			{
				ClientRpcParams clientRpcParams = default(ClientRpcParams);
				FastBufferWriter bufferWriter = __beginSendClientRpc(2310390017u, clientRpcParams, RpcDelivery.Reliable);
				BytePacker.WriteValueBitPacked(bufferWriter, playerWhoTriggered);
				__endSendClientRpc(ref bufferWriter, 2310390017u, clientRpcParams, RpcDelivery.Reliable);
			}
			if (__rpc_exec_stage == __RpcExecStage.Client && (networkManager.IsClient || networkManager.IsHost) && (int)GameNetworkManager.Instance.localPlayerController.playerClientId != playerWhoTriggered && !base.IsServer)
			{
				SpikeTrapSlam();
			}
		}
	...
	{
		NetworkManager.__rpc_func_table.Add(1039429333u, __rpc_handler_1039429333);
		NetworkManager.__rpc_func_table.Add(3844133644u, __rpc_handler_3844133644);
		NetworkManager.__rpc_func_table.Add(4061245259u, __rpc_handler_4061245259);
		NetworkManager.__rpc_func_table.Add(2310390017u, __rpc_handler_2310390017);
	}
	...
	private static void __rpc_handler_1039429333(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			((SpikeRoofTrap)target).ToggleSpikesServerRpc(value);
	...
	private static void __rpc_handler_3844133644(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			((SpikeRoofTrap)target).ToggleSpikesClientRpc(value);
			target.__rpc_exec_stage = __RpcExecStage.None;
		}
	}
	...
	private static void __rpc_handler_4061245259(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			target.__rpc_exec_stage = __RpcExecStage.Server;
			((SpikeRoofTrap)target).SpikeTrapSlamServerRpc(value);
			target.__rpc_exec_stage = __RpcExecStage.None;
		}
	}
	...
	private static void __rpc_handler_2310390017(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			ByteUnpacker.ReadValueBitPacked(reader, out int value);
			target.__rpc_exec_stage = __RpcExecStage.Client;
			((SpikeRoofTrap)target).SpikeTrapSlamClientRpc(value);
			target.__rpc_exec_stage = __RpcExecStage.None;
		}
	}
