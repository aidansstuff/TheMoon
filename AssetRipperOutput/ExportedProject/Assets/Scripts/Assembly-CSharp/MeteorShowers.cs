...
public class MeteorShowers : NetworkBehaviour
{
	...
	public void CreateMeteorServerRpc(float landingTime, Vector3 landingPosition, Vector3 skyDirection, float size)
	{
		...
		if (__rpc_exec_stage != __RpcExecStage.Server && (networkManager.IsClient || networkManager.IsHost))
		{
			...
			FastBufferWriter bufferWriter = __beginSendServerRpc(505987061u, serverRpcParams, RpcDelivery.Reliable);
			bufferWriter.WriteValueSafe(in landingTime, default(FastBufferWriter.ForPrimitives));
			bufferWriter.WriteValueSafe(in landingPosition);
			bufferWriter.WriteValueSafe(in skyDirection);
			bufferWriter.WriteValueSafe(in size, default(FastBufferWriter.ForPrimitives));
			__endSendServerRpc(ref bufferWriter, 505987061u, serverRpcParams, RpcDelivery.Reliable);
		}
	...
	public void CreateMeteorClientRpc(float landingTime, Vector3 landingPosition, Vector3 skyDirection, float size)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			if (__rpc_exec_stage != __RpcExecStage.Client && (networkManager.IsServer || networkManager.IsHost))
			{
				...
				FastBufferWriter bufferWriter = __beginSendClientRpc(663969610u, clientRpcParams, RpcDelivery.Reliable);
				bufferWriter.WriteValueSafe(in landingTime, default(FastBufferWriter.ForPrimitives));
				bufferWriter.WriteValueSafe(in landingPosition);
				bufferWriter.WriteValueSafe(in skyDirection);
				bufferWriter.WriteValueSafe(in size, default(FastBufferWriter.ForPrimitives));
				__endSendClientRpc(ref bufferWriter, 663969610u, clientRpcParams, RpcDelivery.Reliable);
			}
			if (__rpc_exec_stage == __RpcExecStage.Client && (networkManager.IsClient || networkManager.IsHost) && !base.IsServer && TimeOfDay.Instance.timeHasStarted)
			{
	...
	{
		NetworkManager.__rpc_func_table.Add(505987061u, __rpc_handler_505987061);
		NetworkManager.__rpc_func_table.Add(663969610u, __rpc_handler_663969610);
	}
	...
	private static void __rpc_handler_505987061(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		...
		((MeteorShowers)target).CreateMeteorServerRpc(value, value2, value3, value4);
	...
	private static void __rpc_handler_663969610(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
	{
		...
		if ((object)networkManager != null && networkManager.IsListening)
		{
			...
			((MeteorShowers)target).CreateMeteorClientRpc(value, value2, value3, value4);
			target.__rpc_exec_stage = __RpcExecStage.None;
		}
	}
