		...
		private List<NavMeshBuildSource> CollectSources()
		{
			...
			foreach (NavMeshModifier item2 in list2)
			{
				if (((int)m_LayerMask & (1 << item2.gameObject.layer)) != 0 && item2.AffectsAgentType(m_AgentTypeID))
				{
					...
					item.applyToChildren = item2.applyToChildren;
					item.overrideGenerateLinks = item2.overrideGenerateLinks;
					item.generateLinks = item2.generateLinks;
					list.Add(item);
				}
			}
			if (m_CollectObjects == CollectObjects.All)
			{
				NavMeshBuilder.CollectSources(null, m_LayerMask, m_UseGeometry, m_DefaultArea, m_GenerateLinks, list, includeOnlyMarkedObjects: false, sources);
			}
			else if (m_CollectObjects == CollectObjects.Children)
			{
				NavMeshBuilder.CollectSources(base.transform, m_LayerMask, m_UseGeometry, m_DefaultArea, m_GenerateLinks, list, includeOnlyMarkedObjects: false, sources);
			}
			else if (m_CollectObjects == CollectObjects.Volume)
			{
				NavMeshBuilder.CollectSources(GetWorldBounds(Matrix4x4.TRS(base.transform.position, base.transform.rotation, Vector3.one), GetInflatedBounds()), m_LayerMask, m_UseGeometry, m_DefaultArea, m_GenerateLinks, list, includeOnlyMarkedObjects: false, sources);
			}
			else if (m_CollectObjects == CollectObjects.MarkedWithModifier)
			{
				NavMeshBuilder.CollectSources(null, m_LayerMask, m_UseGeometry, m_DefaultArea, m_GenerateLinks, list, includeOnlyMarkedObjects: true, sources);
			}
