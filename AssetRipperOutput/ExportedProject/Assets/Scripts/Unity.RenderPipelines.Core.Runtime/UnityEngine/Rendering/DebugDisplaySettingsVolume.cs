	...
	public class DebugDisplaySettingsVolume : IDebugDisplaySettingsData, IDebugDisplaySettingsQuery
	{
		...
		internal static class WidgetFactory
		{
			...
			private static DebugUI.Widget CreateVolumeParameterWidget(string name, VolumeParameter param, Func<bool> isHiddenCallback = null)
			{
				...
				{
					ColorParameter p2 = (ColorParameter)param;
				...
				{
					BoolParameter p = (BoolParameter)param;
				...
				if (method == null || method.DeclaringType == typeof(object) || method.DeclaringType == typeof(Object))
				{
					...
					return new DebugUI.Value
					{
						...
						getter = delegate
						{
							object value3 = property.GetValue(param);
							return (value3 == null || value3.Equals(null)) ? Strings.none : (nameProp.GetValue(value3) ?? Strings.none);
						},
						isHiddenCallback = isHiddenCallback
					};
				}
				return new DebugUI.Value
				{
					...
					getter = delegate
					{
						object value2 = property.GetValue(param);
						return (value2 != null) ? value2.ToString() : Strings.none;
					},
					isHiddenCallback = isHiddenCallback
				};
			}
			...
			public static DebugUI.Table CreateVolumeTable(DebugDisplaySettingsVolume data)
			{
				...
				{
					table.SetColumnVisibility(j + 1, data.volumeDebugSettings.VolumeHasInfluence(volumes[j]));
				}
				float timer = 0f;
				...
				table.isHiddenCallback = delegate
				{
					...
					if (timer >= refreshRate)
					{
						if (data.volumeDebugSettings.selectedCamera != null)
						{
							...
							{
								for (int l = 0; l < volumes2.Length; l++)
								{
									bool visible = data.volumeDebugSettings.VolumeHasInfluence(volumes2[l]);
									table.SetColumnVisibility(l + 1, visible);
								}
							}
