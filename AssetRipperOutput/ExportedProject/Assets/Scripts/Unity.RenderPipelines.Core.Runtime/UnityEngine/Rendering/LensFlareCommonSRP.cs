...
namespace UnityEngine.Rendering
{
	public sealed class LensFlareCommonSRP
	{
		...
		public static float ShapeAttenuationAreaTubeLight(Vector3 lightPositionWS, Vector3 lightSide, float lightWidth, Camera cam)
		{
			...
			Vector3 p3 = cam.transform.InverseTransformPoint(position);
			Vector3 p4 = cam.transform.InverseTransformPoint(position2);
			Vector3 p5 = cam.transform.InverseTransformPoint(position3);
			Vector3 p6 = cam.transform.InverseTransformPoint(position4);
			float num = DiffLineIntegral(p5, p6);
			float num2 = DiffLineIntegral(p3, p4);
			if (!(num > 0f))
			{
				return 1f;
			}
			return num2 / num;
			static float DiffLineIntegral(Vector3 p1, Vector3 p2)
			{
				...
				if ((double)p2.z < 0.0)
				{
					p2 = (-p1 * p2.z + p2 * p1.z) / (0f - p2.z + p1.z);
				}
				float num3 = Vector3.Dot(p1, normalized);
				float l2 = Vector3.Dot(p2, normalized);
				Vector3 vector = p1 - num3 * normalized;
				float magnitude = vector.magnitude;
				return ((Fpo(magnitude, l2) - Fpo(magnitude, num3)) * vector.z + (Fwt(magnitude, l2) - Fwt(magnitude, num3)) * normalized.z) / MathF.PI;
			}
		...
		public static bool IsCloudLayerOpacityNeeded(Camera cam)
		{
			...
			foreach (LensFlareCompInfo datum in Instance.Data)
			{
				if (datum != null && !(datum.comp == null))
				{
					...
					if (comp.enabled && comp.gameObject.activeSelf && comp.gameObject.activeInHierarchy && !(lensFlareData == null) && lensFlareData.elements != null && lensFlareData.elements.Length != 0 && comp.useOcclusion && (!comp.useOcclusion || comp.sampleCount != 0) && !(comp.intensity <= 0f) && comp.useBackgroundCloudOcclusion)
					{
						return true;
					}
				}
		...
		public static void ComputeOcclusion(Material lensFlareShader, Camera cam, float actualWidth, float actualHeight, bool usePanini, float paniniDistance, float paniniCropToFit, bool isCameraRelative, Vector3 cameraPositionWS, Matrix4x4 viewProjMatrix, CommandBuffer cmd, bool taaEnabled, bool hasCloudLayer, Texture cloudOpacityTexture, Texture sunOcclusionTexture, int _FlareOcclusionTex, int _FlareCloudOpacity, int _FlareOcclusionIndex, int _FlareTex, int _FlareColorValue, int _FlareSunOcclusionTex, int _FlareData0, int _FlareData1, int _FlareData2, int _FlareData3, int _FlareData4)
		{
			...
			foreach (LensFlareCompInfo datum in m_Data)
			{
				...
				LensFlareComponentSRP comp = datum.comp;
				LensFlareDataSRP lensFlareData = comp.lensFlareData;
				if (!comp.enabled || !comp.gameObject.activeSelf || !comp.gameObject.activeInHierarchy || lensFlareData == null || lensFlareData.elements == null || lensFlareData.elements.Length == 0 || comp.intensity <= 0f || !comp.useOcclusion || (comp.useOcclusion && comp.sampleCount == 0))
				{
					continue;
				}
				...
				if (!(Vector3.Dot(cam.transform.forward, rhs) < 0f))
				{
					...
					Vector3 vector4 = WorldToViewport(cam, !flag, isCameraRelative, viewProjMatrix, vector2 + normalized * comp.occlusionOffset);
					float num3 = (flag ? comp.celestialProjectedOcclusionRadius(cam) : comp.occlusionRadius);
					Vector2 vector5 = vector3;
					float magnitude2 = ((Vector2)WorldToViewport(cam, !flag, isCameraRelative, viewProjMatrix, vector2 + cam.transform.up * num3) - vector5).magnitude;
					...
					if (!(comp.intensity * num4 * num2 <= 0f))
					{
						...
						rect.x = datum.index;
						rect.y = (frameIdx + mergeNeeded) * num;
						rect.width = 1f;
						rect.height = 1f;
						Rect viewport = rect;
		...
		public static void DoLensFlareDataDrivenCommon(Material lensFlareShader, Camera cam, float actualWidth, float actualHeight, bool usePanini, float paniniDistance, float paniniCropToFit, bool isCameraRelative, Vector3 cameraPositionWS, Matrix4x4 viewProjMatrix, CommandBuffer cmd, bool taaEnabled, bool hasCloudLayer, Texture cloudOpacityTexture, Texture sunOcclusionTexture, RenderTargetIdentifier colorBuffer, Func<Light, Camera, Vector3, float> GetLensFlareLightAttenuation, int _FlareOcclusionRemapTex, int _FlareOcclusionTex, int _FlareOcclusionIndex, int _FlareCloudOpacity, int _FlareSunOcclusionTex, int _FlareTex, int _FlareColorValue, int _FlareData0, int _FlareData1, int _FlareData2, int _FlareData3, int _FlareData4, bool debugView)
		{
			...
			foreach (LensFlareCompInfo datum in m_Data)
			{
				...
				LensFlareComponentSRP comp = datum.comp;
				LensFlareDataSRP lensFlareData = comp.lensFlareData;
				if (!comp.enabled || !comp.gameObject.activeSelf || !comp.gameObject.activeInHierarchy || lensFlareData == null || lensFlareData.elements == null || lensFlareData.elements.Length == 0 || comp.intensity <= 0f)
				{
					continue;
				}
				Light component = comp.GetComponent<Light>();
				...
				foreach (LensFlareDataElementSRP element in elements)
				{
					...
					if (!element.allowMultipleElement || element.count == 1)
					{
						...
						if (element.enableRadialDistortion)
						{
							...
							curSize2 = ComputeLocalSize(lensFlareRayOffset, lensFlareRayOffset2, curSize2, element.distortionCurve);
						}
					...
					if (element.distribution == SRPLensFlareDistribution.Uniform)
					{
						...
						for (int j = 0; j < element.count; j++)
						{
							...
							if (element.enableRadialDistortion)
							{
								...
								curSize3 = ComputeLocalSize(lensFlareRayOffset3, lensFlareRayOffset4, curSize3, element.distortionCurve);
							}
					...
					else
					{
						...
						for (int l = 0; l < element.count; l++)
						{
							...
							Vector2 curSize4 = vector8;
							if (element.enableRadialDistortion)
							{
								Vector2 lensFlareRayOffset8 = GetLensFlareRayOffset(screenPos, 0f, globalCos0, globalSin0);
								curSize4 = ComputeLocalSize(lensFlareRayOffset7, lensFlareRayOffset8, curSize4, element.distortionCurve);
							}
							...
							cmd.SetGlobalVector(_FlareData2, new Vector4(screenPos.x, screenPos.y, curSize4.x, curSize4.y));
					...
					Vector2 ComputeLocalSize(Vector2 rayOff, Vector2 rayOff0, Vector2 curSize, AnimationCurve distortionCurve)
					{
						...
						return new Vector2(Mathf.Lerp(curSize.x, element.targetSizeDistortion.x * combinedScale / usedAspectRatio, t), Mathf.Lerp(curSize.y, element.targetSizeDistortion.y * combinedScale, t));
					}
				}
			}
