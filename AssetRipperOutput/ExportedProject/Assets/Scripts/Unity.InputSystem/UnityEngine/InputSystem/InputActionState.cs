...
namespace UnityEngine.InputSystem
{
	internal class InputActionState : IInputStateChangeMonitor, ICloneable, IDisposable
	{
				...
				for (int k = 0; k < reference3.interactionCount; k++)
				{
					...
					if (control != null)
					{
						...
						if (reference7.isTimerRunning)
						{
							...
							triggerState.mapIndex = reference3.mapIndex;
							triggerState.controlIndex = num;
							triggerState.bindingIndex = j;
							triggerState.time = reference7.timerStartTime;
							triggerState.interactionIndex = reference3.interactionStartIndex + k;
		...
		public unsafe void ResetActionState(int actionIndex, InputActionPhase toPhase = InputActionPhase.Waiting, bool hardReset = false)
		{
			...
			if (ptr->phase != InputActionPhase.Waiting && ptr->phase != 0)
			{
		...
		private unsafe void ProcessControlStateChange(int mapIndex, int controlIndex, int bindingIndex, double time, InputEventPtr eventPtr)
		{
			using (InputActionRebindingExtensions.DeferBindingResolution())
			{
				...
				try
				{
					...
					triggerState.isButton = actionIndex != -1 && actionStates[actionIndex].isButton;
					TriggerState trigger = triggerState;
					if (m_OnBeforeUpdateHooked)
					{
			...
			if (actionIndex != -1)
			{
				...
				else if (newPhase == InputActionPhase.Canceled && actionStates[actionIndex].interactionIndex == trigger.interactionIndex)
				{
					...
					for (int i = 0; i < interactionCount; i++)
					{
						...
						triggerState.mapIndex = trigger.mapIndex;
						...
						if (ChangePhaseOfAction(InputActionPhase.Started, ref trigger2))
						{
							...
							triggerState = default(TriggerState);
							triggerState.phase = InputActionPhase.Performed;
							triggerState.controlIndex = interactionStates[num].triggerControlIndex;
							triggerState.bindingIndex = trigger.bindingIndex;
							triggerState.interactionIndex = num;
							triggerState.mapIndex = trigger.mapIndex;
							triggerState.time = interactionStates[num].performedTime;
							triggerState.startTime = startTime;
							trigger2 = triggerState;
