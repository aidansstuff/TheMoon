...
namespace UnityEngine.InputSystem.Utilities
{
	internal struct InlinedArray<TValue> : IEnumerable<TValue>, IEnumerable
	{
		...
		{
			InlinedArray<TValue> result = default(InlinedArray<TValue>);
			result.length = length;
			result.firstValue = firstValue;
			result.additionalValues = ((additionalValues != null) ? ArrayHelpers.Copy(additionalValues) : null);
			return result;
		}
		...
		public int IndexOf(TValue value)
		{
			...
			if (length > 0)
			{
				if (@default.Equals(firstValue, value))
				{
					return 0;
				}
				if (additionalValues != null)
				{
					for (int i = 0; i < length - 1; i++)
					{
						if (@default.Equals(additionalValues[i], value))
						{
							return i + 1;
						}
					}
