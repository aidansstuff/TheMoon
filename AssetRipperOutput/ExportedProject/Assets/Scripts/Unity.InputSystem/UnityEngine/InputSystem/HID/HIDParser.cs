...
namespace UnityEngine.InputSystem.HID
{
	internal static class HIDParser
	{
		...
		{
			HIDItemStateLocal state = default(HIDItemStateLocal);
			...
			while (ptr2 < ptr)
			{
				...
				case 100:
					...
					break;
				case 8:
					state.SetUsage(ReadData(b, ptr2, ptr));
					break;
				case 24:
					state.usageMinimum = ReadData(b, ptr2, ptr);
					break;
				case 40:
					state.usageMaximum = ReadData(b, ptr2, ptr);
					break;
				case 160:
				{
					int parent = num;
					...
					list3.Add(new HID.HIDCollectionDescriptor
					{
						...
						parent = parent,
						usagePage = hIDItemStateGlobal.GetUsagePage(0, ref state),
						usage = state.GetUsage(0),
						firstChild = list2.Count
					});
					HIDItemStateLocal.Reset(ref state);
					break;
				}
				case 192:
				{
				...
				case 176:
				{
					...
					for (int i = 0; i < valueOrDefault; i++)
					{
						...
						hIDElementDescriptor.usage = state.GetUsage(i) & 0xFFFF;
						hIDElementDescriptor.usagePage = hIDItemStateGlobal.GetUsagePage(i, ref state);
						hIDElementDescriptor.reportType = reportType;
						hIDElementDescriptor.reportSizeInBits = hIDItemStateGlobal.reportSize.GetValueOrDefault(8);
						hIDElementDescriptor.reportOffsetInBits = value.currentBitOffset;
						hIDElementDescriptor.reportId = hIDItemStateGlobal.reportId.GetValueOrDefault(1);
						hIDElementDescriptor.flags = (HID.HIDElementFlags)flags;
						hIDElementDescriptor.logicalMin = hIDItemStateGlobal.logicalMinimum.GetValueOrDefault(0);
						hIDElementDescriptor.logicalMax = hIDItemStateGlobal.logicalMaximum.GetValueOrDefault(0);
						hIDElementDescriptor.physicalMin = hIDItemStateGlobal.GetPhysicalMin();
						hIDElementDescriptor.physicalMax = hIDItemStateGlobal.GetPhysicalMax();
						hIDElementDescriptor.unitExponent = hIDItemStateGlobal.unitExponent.GetValueOrDefault(0);
						hIDElementDescriptor.unit = hIDItemStateGlobal.unit.GetValueOrDefault(0);
						HID.HIDElementDescriptor item = hIDElementDescriptor;
