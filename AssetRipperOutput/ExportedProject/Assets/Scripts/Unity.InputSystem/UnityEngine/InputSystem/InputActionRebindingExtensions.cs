	...
	public static class InputActionRebindingExtensions
	{
		...
		internal struct ParameterOverride
		{
			...
			public static ParameterOverride? Find(InputActionMap actionMap, ref InputBinding binding, string parameterName, string objectRegistrationName)
			{
				...
				ParameterOverride? second = ((asset != null) ? Find(asset.m_ParameterOverrides, asset.m_ParameterOverridesCount, ref binding, parameterName, objectRegistrationName) : null);
		...
		public static PrimitiveValue? GetParameterValue(this InputAction action, string name, int bindingIndex)
		{
			...
			inputBinding.id = action.GetOrCreateActionMap().bindings[index].id;
			InputBinding bindingMask = inputBinding;
		...
		public static string SaveBindingOverridesAsJson(this IInputActionCollection2 actions)
		{
			...
			InputActionMap.BindingOverrideListJson bindingOverrideListJson = default(InputActionMap.BindingOverrideListJson);
			bindingOverrideListJson.bindings = list;
			return JsonUtility.ToJson(bindingOverrideListJson);
		}
